...
Shader "3dArthings/LWRP/MatcapFakePBR/Simple/BlendTintColor"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
				...
				float4 lerpResult47 = lerp( tex2D( _MainTexture01, ( ( mul( UNITY_MATRIX_V, float4( WorldNormal , 0.0 ) ).xyz * 0.5 ) + 0.5 ).xy ) , tex2D( _MainTexture02, ( ( mul( UNITY_MATRIX_V, float4( WorldNormal , 0.0 ) ).xyz * 0.5 ) + 0.5 ).xy ) , _BlendTextures);
				...
				float3 Albedo = (( _ActivateTINT )?( lerpResult62 ):( (( _DesaturateIT )?( float4( desaturateVar59 , 0.0 ) ):( lerpResult47 )) )).rgb;
				float3 Normal = float3(0, 0, 1);
				float3 Emission = lerpResult50.rgb;
				float3 Specular = 0.5;
				float Metallic = 0;
