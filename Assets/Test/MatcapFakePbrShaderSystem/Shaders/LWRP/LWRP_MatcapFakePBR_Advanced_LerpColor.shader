...
Shader "3dArthings/LWRP/MatcapFakePBR/Advanced/LerpColor"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
				...
				float3 desaturateInitialColor79 = tex2D( _MainTexture, ( ( mul( UNITY_MATRIX_V, ( tex2DNode51 + float4( WorldNormal , 0.0 ) ) ) * 0.5 ) + 0.5 ).rg ).rgb;
				...
				float4 lerpResult80 = lerp( _Color2 , _Color3 , float4( desaturateVar79 , 0.0 ));
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				float3 desaturateInitialColor79 = tex2D( _MainTexture, ( ( mul( UNITY_MATRIX_V, ( tex2DNode51 + float4( ase_worldNormal , 0.0 ) ) ) * 0.5 ) + 0.5 ).rg ).rgb;
				float desaturateDot79 = dot( desaturateInitialColor79, float3( 0.299, 0.587, 0.114 ));
				float3 desaturateVar79 = lerp( desaturateInitialColor79, desaturateDot79.xxx, 1.0 );
				float4 lerpResult80 = lerp( _Color2 , _Color3 , float4( desaturateVar79 , 0.0 ));
				float4 blendOpSrc52 = tex2D( _AOMap, uv_AOMap );
